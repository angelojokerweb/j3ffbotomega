		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp4':
					if (args.length < 1) return reply('Onde está o url, hum?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://https://mhankbarbar.tech/api/yta?url=${args[0]}&apiKey=${BarBarKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               case 'thiago':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://giphy.com/search/Frog`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Thiago meu amor 😍'})
					break
               case 'dono':
					memein = await kagApi.memeindo()
